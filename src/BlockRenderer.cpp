...
BlockRenderer::BlockRenderer(model::ModelRenderer &modelRenderer) : simpleBlockShader("simpleBlockShader"), modelRenderer(modelRenderer)
{
	...
	{
		cyberCubes::model::ModelChest modelChest;
		modelChest.textureOffset = modelRenderer.getModelTextures().getTextureTile("chest").position;
		std::vector<cyberCubes::model::Vertex> verticies;
		modelChest.render(verticies, invTextureSize);
		model::ModelRendererBuffer &modelRendererBuffer = modelBuffers[BLOCK_CHEST];
		modelRendererBuffer.upload(verticies);
	}
}
...
void BlockRenderer::renderInventoryBlock(const Block *block, const math::mat4 &mvp)
{
	...
	{
		auto it = modelBuffers.find(block->id);
		if (it != modelBuffers.end())
		{
			modelRenderer.startRender();
			modelRenderer.render(it->second, mvp);
